...
| [Enforce Upload Time Window](https://github.com/cloudsmith-io/rego-recipes/tree/main?tab=readme-ov-file#recipe-16---suspicious-package-upload-window)                                  | Allow uploads during business hours (9 AM – 5 PM UTC), to catch anomalous behavior like late-night uploads                                      |
...
| [Hugging Face Recipes](https://github.com/cloudsmith-io/rego-recipes/blob/main/huggingface-recipes/README.md/)                                                                         | Policies relating to Hugging Face models/datasets.                                                                                              |
...
cat <<EOF > payload.json
{
  ...
  "description": "Flag packages that have more than a threshold number of tags to avoid ungoverned tagging behaviors.",
  "rego": $escaped_policy,
  "enabled": true,
  "is_terminal": false,
  "precedence": 5
...
Assuming your organization don't expect packages to be uploaded or modified at specific times, the below policy can detect package uploads at `9am-11am UTC` - regardless of the package name. <br/>
Download the `policy.rego` and create the associated `payload.json` with the below command:
